...
class DICOMForm(ModelForm):
    class Meta:
        model = DICOM
        fields = ['dicom_setting']

        widgets = {
            'dicom_setting': Select(attrs={'class': 'form-control', 'required': "",
                                         'data-error': _('DICOM setting type must be filled.')})
        }
...
class DICOMSettingForm(ModelForm):
    class Meta:
        model = DICOMSetting
        ...
        widgets = {
            ...
            'description': Textarea(attrs={'class': 'form-control',
                                           'rows': '4', 'required': "",
                                           'data-error': _('Description must be filled.')}),
            # 'archivo': FileField(widget=FileInput(attrs={'class': 'control-label'}))
...
class DICOMForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        initial = kwargs.get('initial')
        if initial:
            self.fields['dicom_setting'].queryset = DICOMSetting.objects.filter(experiment=initial['experiment'])
